...
void drawBlackHole()
{
    ...
    // Re-initialize particles if scale changed significantly (for animations)
    else if (abs(scale - previousEventHorizonRadius / 14.0f) > 0.1f)
    {
        for (int i = 0; i < MAX_ACCRETION_PARTICLES; i++)
        {
            initializeAccretionParticle(i, centerX, centerY);
    ...
    if (deltaTime > 0.1f)
        deltaTime = 0.1f; // Cap to avoid huge jumps
    if (deltaTime <= 0)
        deltaTime = 0.016; // Prevent division by zero or negative time if millis() wraps or glitches
    blackHoleLastUpdateTime = currentTime;
    ...
    for (int i = 0; i < MAX_ACCRETION_PARTICLES; i++)
    {
        if (accretionDisk[i].prevX >= 0)
        { // Check if it had a valid previous position
    ...
    for (int i = 0; i < MAX_ACCRETION_PARTICLES; i++)
    {
        ...
        if (accretionDisk[i].angle < 0)
            accretionDisk[i].angle += 2.0f * PI;
    ...
    for (int i = 0; i < MAX_FALLING_STARS; i++)
    {
        ...
        if (dist <= blackHoleRadius || x < -10 || x >= SCREEN_WIDTH + 10 || y < -10 || y >= SCREEN_HEIGHT + 10)
        { // Wider bounds check
            fallingStars[i].active = false;

            // If star was consumed (close enough), trigger consumption effect and trail fade
            if (dist <= blackHoleRadius * 1.5f)
            { // Only flash if consumed near BH
    ...
    { // Reduced frequency
        for (int i = 0; i < MAX_FALLING_STARS; i++)
        {
            if (!fallingStars[i].active && !fallingStars[i].hasTrail)
            { // Only activate if truly inactive
                ...
                {
                case 0:
                    fallingStars[i].x = random(SCREEN_WIDTH);
                    fallingStars[i].y = -5;
                    break; // Start slightly off screen
                case 1:
                    fallingStars[i].x = SCREEN_WIDTH + 4;
                    fallingStars[i].y = random(SCREEN_HEIGHT);
                    break;
                case 2:
                    fallingStars[i].x = random(SCREEN_WIDTH);
                    fallingStars[i].y = SCREEN_HEIGHT + 4;
                    break;
                case 3:
                    fallingStars[i].x = -5;
                    fallingStars[i].y = random(SCREEN_HEIGHT);
    ...
    for (int i = 0; i < MAX_ACCRETION_PARTICLES; i++)
    {
        if (!accretionDisk[i].active || accretionDisk[i].prevX < 0)
            continue; // Skip inactive or invalid position
        if (sin(accretionDisk[i].angle) > 0)
            continue; // Skip front half
        ...
        if (x >= 0 && x < SCREEN_WIDTH && y >= 0 && y < SCREEN_HEIGHT)
        {
            ...
            if (distSqFromCenter <= sq(round(blackHoleRadius)))
            {             // Use sq(round(...)) for consistency with fillCircle
            ...
            for (int t = 0; t < accretionDisk[i].trailLength; t++)
            {
                ...
                if (trailX < 0 || trailY < 0 || trailX >= SCREEN_WIDTH || trailY >= SCREEN_HEIGHT)
                    continue;

                float trailDistSq = sq(trailX - centerX) + sq(trailY - centerY);
                if (trailDistSq <= sq(round(blackHoleRadius)))
                    continue;
    ...
    {   // Draw if radius is at least half a pixel
    ...
    if (blackHoleRadius >= 0.5)
    {
        ...
        for (float angle = PI * 0.75f; angle < PI * 1.25f; angle += 0.04f)
        { // Adjusted angle range/step
        ...
        if (r_bh + 1 < min(SCREEN_WIDTH, SCREEN_HEIGHT) / 2)
        { // Basic check to avoid huge circles
