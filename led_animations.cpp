...
void _updateMenuEffect()
{
    ...
    case 0:
        ...
        break; // Discovery
    case 1:
        menuColor = CRGB::Yellow;
        break; // Quiz
    case 2:
        menuColor = CRGB::Red;
        break; // Story
    default:
        menuColor = CRGB::White;
        break;
    }
    leds[1] = ledOn ? menuColor : CRGB::Black;
...
void _updateQuizEffect()
{
        ...
        if (NUM_LEDS > 2)
        {
            ...
            for (int i = 0; i < NUM_LEDS; ++i)
            {
                ...
                else if (dist < 2.5f)
                {
                    ...
                    if (tailVal > 10)
                        leds[i] += CHSV(baseHue + 8, 220, tailVal);
                }
            }
        }
        return;
    ...
    if (quizWaiting)
    {
        ...
        if (currentTime - lastStepTime > 150)
        { // Slower chase
    ...
    { // Answered
        ...
        if (step < flashDuration * 2)
        { // Total steps for on/off cycles
...
void _updateStoryEffect()
{
    ...
    if (storyBreathPhase > TWO_PI)
        storyBreathPhase -= TWO_PI;
...
void _updateTransitionEffect()
{
    ...
    if (elapsedTime >= TRANSITION_DURATION)
    {
        ...
        if (currentLedMode == LedMode::MENU)
            _updateMenuEffect();
        ...
        if (finalMode == LedMode::MENU)
            _updateMenuEffect();
        else if (finalMode == LedMode::QUIZ)
            _updateQuizEffect();
        else if (finalMode == LedMode::STORY)
            _updateStoryEffect();
        else if (finalMode == LedMode::WARP)
            _updateWarpEffect();
        else if (finalMode == LedMode::DISCOVERY)
            _updateDiscoveryEffect();
        else
        {
            FastLED.clear();
    ...
    else
    {
        ...
        if (previousLedMode == LedMode::MENU)
            _updateMenuEffect();
        else if (previousLedMode == LedMode::QUIZ)
            _updateQuizEffect();
        else if (previousLedMode == LedMode::STORY)
            _updateStoryEffect();
        else if (previousLedMode == LedMode::WARP)
            _updateWarpEffect();
        else if (previousLedMode == LedMode::DISCOVERY)
            _updateDiscoveryEffect();
        else
        {
            leds[0] = CRGB::Black;
