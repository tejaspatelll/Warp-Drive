...
float fastSqrt(float value) {
    if (value <= 0) return 0;
    if (value >= MAX_HALO_RADIUS * MAX_HALO_RADIUS) return MAX_HALO_RADIUS;
...
void drawNebula() {
    ...
    // Initialize nebula if not already done
    if (!nebulaInitialized) {
        ...
        if (!nebulaParticles || !nebulaCores || !nebulaFilaments) {
            ...
            if (nebulaParticles) free(nebulaParticles);
            if (nebulaCores) free(nebulaCores);
            if (nebulaFilaments) free(nebulaFilaments);
        ...
        for (int i = 0; i < coreParticles; i++) {
            ...
            nebulaParticles[i].haloSize = haloRand < 60 ? 
                1.0 + random(min(20, MAX_HALO_RADIUS * 10 - 10)) / 10.0 : 0; // 60% with halo
    ...
    if (deltaTime > 0.1f) deltaTime = 0.1f; // Cap delta time to prevent jumps
    ...
    for (int i = 0; i < particlesToUpdate; i++) {
        ...
        if (particle.filamentIndex < 0) { // Only for core-based particles
            for (int c = 0; c < MAX_NEBULA_CORES; c++) {
                float dx = nebulaCores[c].x - particle.x;
                float dy = nebulaCores[c].y - particle.y;
                float dist = sqrt(dx*dx + dy*dy);
            ...
            if (particle.haloSize > 0) {
                ...
                for (int p = 0; p < haloPointCount; p++) {
                    if (haloOffsets[p].dist <= particle.haloSize) {
                        ...
                        float alpha = haloOffsets[p].dist == 0 ? 1.0 : 
                                    (1.0 - haloOffsets[p].dist/particle.haloSize);
