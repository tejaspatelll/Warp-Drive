...
void drawMenu() {
  ...
  // Draw animated starfield background for menu using global stars
  updateStars();

  // Menu layout parameters
  int baseCardWidth = SCREEN_WIDTH * 0.7;
  int baseCardHeight = SCREEN_HEIGHT * 0.13;
  int cardSpacing = SCREEN_HEIGHT * 0.04;
  int totalHeight = MENU_ITEMS * baseCardHeight + (MENU_ITEMS - 1) * cardSpacing;
  int startY = (SCREEN_HEIGHT - totalHeight) / 2;

  // Draw each menu item as a card
  for (int i = 0; i < MENU_ITEMS; i++) {
    // Calculate card dimensions (enlarged if selected)
    float scaleFactor = (i == currentMenuItem) ? 1.15f : 1.0f; // Enlarge selected
    ...
    if (i == currentMenuItem) {
      // Draw shadow
      tft.fillRoundRect(cardX + 4, cardY + 6, cardWidth, cardHeight, 12, tft.color565(30, 30, 60));
      // Draw gradient highlight (simulate with two rectangles)
      tft.fillRoundRect(cardX, cardY, cardWidth, cardHeight, 12, tft.color565(60, 0, 120));
      tft.fillRoundRect(cardX, cardY, cardWidth, cardHeight / 2, 12, tft.color565(120, 0, 200));
      // Draw border
      tft.drawRoundRect(cardX, cardY, cardWidth, cardHeight, 12, COLOR_HIGHLIGHT);
    } else {
      // Dimmed card for unselected items
      tft.fillRoundRect(cardX, cardY, cardWidth, cardHeight, 12, tft.color565(20, 20, 40));
      tft.drawRoundRect(cardX, cardY, cardWidth, cardHeight, 12, COLOR_BORDER_ALT);
    }

    // Draw icon (simple shape for now)
    int iconX = cardX + cardHeight / 2;
    int iconY = cardY + cardHeight / 2;
    int iconSize = cardHeight / 4;
    switch (i) {
      case 0: // Discovery
      ...
      case 1: // Quiz
        ...
        break;
      case 2: // Story
        tft.drawTriangle(iconX - iconSize / 2, iconY + iconSize / 2,
                         iconX + iconSize / 2, iconY + iconSize / 2,
                         iconX, iconY - iconSize / 2, tft.color565(255, 100, 100));
        break;
    }
    ...
    if (i == currentMenuItem) {
      tft.setTextColor(COLOR_HIGHLIGHT);
      tft.setTextSize(2 * scaleFactor); // Scale text size
    } else {
      tft.setTextColor(COLOR_TEXT);
      tft.setTextSize(1 * scaleFactor);
    }
    tft.drawString(menuItems[i].name, cardX + cardWidth / 2 + cardHeight / 4, cardY + cardHeight / 2);
  }

  // Draw footer bar for instructions
  int footerHeight = SCREEN_HEIGHT * 0.08;
  tft.fillRect(0, SCREEN_HEIGHT - footerHeight, SCREEN_WIDTH, footerHeight, tft.color565(0, 30, 60));
  tft.setTextColor(COLOR_GREEN);
  tft.setTextSize(1);
  tft.setTextDatum(MC_DATUM);
  tft.drawString("KNOB: Select  |  BTN: Go", SCREEN_WIDTH / 2, SCREEN_HEIGHT - footerHeight / 2);

  // Optionally, add a logo or title at the top
  tft.setTextColor(tft.color565(0, 200, 255));
  tft.setTextSize(2);
  tft.setTextDatum(MC_DATUM);
  tft.drawString("Cosmic Knobulator", SCREEN_WIDTH / 2, SCREEN_HEIGHT * 0.08);
}
