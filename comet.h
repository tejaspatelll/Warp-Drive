...
namespace
{
  ...
  bool cometHeadExited = false;       // Track if comet head has exited screen
...
void drawComet()
{
  ...
  if (!cometInitialized)
  {
    ...
    float speed = (0.3f + random(0, 50) / 500.0f) * scale;
  ...
  // Draw nucleus with glow only if head hasn't exited
  if (!cometHeadExited && x >= 0 && x < SCREEN_WIDTH && y >= 0 && y < SCREEN_HEIGHT)
  {
  ...
  // Spawn new tail particles at a faster rate (only if head hasn't exited)
  if (!cometHeadExited && currentTime - cometLastParticleTime > 5)
  {
    for (int i = 0; i < MAX_COMET_TAIL; i++)
    {
  ...
  if (!cometHeadExited && (x < -cometRadius || x > SCREEN_WIDTH + cometRadius ||
                           y < -cometRadius || y > SCREEN_HEIGHT + cometRadius))
  {
    // Mark head as exited but don't reset yet
    cometHeadExited = true;
  }

  // Check if all tail particles have disappeared
  bool allTailParticlesGone = true;
  for (int i = 0; i < MAX_COMET_TAIL; i++)
  {
    if (cometTail[i].brightness > 0)
    {
      allTailParticlesGone = false;
      break;
    }
  }

  // Only reset comet when head has exited AND all tail particles are gone
  if (cometHeadExited && allTailParticlesGone)
  {
    // Erase nucleus
    if (prevCometX >= 0 && prevCometX < SCREEN_WIDTH &&
        prevCometY >= 0 && prevCometY < SCREEN_HEIGHT)
    ...
    // Erase any remaining tail particles
    for (int i = 0; i < MAX_COMET_TAIL; i++)
    {
      if (cometTail[i].brightness > 0)
      {
