...
void initializeOrion() {
    ...
    if (requiredSize > availablePSRAM) {
        ...
        if (numOrionParticles < 50) numOrionParticles = 50; // Ensure a minimum
    ...
    for (int i = 0; i < numOrionParticles; i++) {
        ...
        if (angle > PI/4 && angle < 3*PI/4) { // Upper wing
            wingFactor = 1.3f + 0.4f * sin((angle - PI/4) * 2); 
        } else if (angle > 5*PI/4 && angle < 7*PI/4) { // Lower wing
    ...
    for (int i = MAX_TRAPEZIUM_STARS; i < MAX_TRAPEZIUM_STARS + MAX_BACKGROUND_STARS; i++) {
        ...
        if (random(100) < 25) { // 25% are protostars/young stellar objects
    ...
    for (int i = 0; i < MAX_PROTOPLANETARY_DISKS; i++) {
        ...
        if (!protoDisks[i].active) continue;
    ...
    for (int i = 0; i < MAX_HERBIG_HARO_OBJECTS; i++) {
        ...
        if (!hhObjects[i].active) continue;
...
void drawOrion() {
    ...
    if (globalSwirl > 2 * PI) globalSwirl -= 2 * PI;
    ...
    for (int i = 0; i < numOrionParticles; i++) {
        if (!orionParticles[i].active) continue;
        ...
        for (int j = 0; j < MAX_TRAPEZIUM_STARS; j++) {
            if (!orionStars[j].active) continue;
    ...
    for (int i = 0; i < MAX_PROTOPLANETARY_DISKS; i++) {
        ...
        if (protoDisks[i].diskAngle > 2 * PI) protoDisks[i].diskAngle -= 2 * PI;
    ...
    // Draw Herbig-Haro jets (middle layer)
    for (int i = 0; i < MAX_HERBIG_HARO_OBJECTS; i++) {
        if (!hhObjects[i].active) continue;
        
        // Update jet motion
        hhObjects[i].jetPhase += hhObjects[i].jetSpeed * deltaTime;
        if (hhObjects[i].jetPhase > 2 * PI) hhObjects[i].jetPhase -= 2 * PI;
        
        // Draw jet as extending line with varying brightness
        float jetX = centerX + hhObjects[i].originalX * scale;
        float jetY = centerY + hhObjects[i].originalY * scale;
        
        float jetLength = 8.0f + 3.0f * sin(hhObjects[i].jetPhase);
        
        // Draw jet in both directions
        for (int dir = -1; dir <= 1; dir += 2) {
            float endX = jetX + cos(hhObjects[i].jetAngle) * jetLength * scale * dir;
            float endY = jetY + sin(hhObjects[i].jetAngle) * jetLength * scale * dir;
            
            if (endX >= 0 && endX < SCREEN_WIDTH && endY >= 0 && endY < SCREEN_HEIGHT) {
                uint16_t jetColor = hhObjects[i].jetColor;
                float pulseBrightness = hhObjects[i].brightness * (0.7f + 0.3f * sin(hhObjects[i].jetPhase));
                
                uint8_t r5 = ((jetColor >> 11) & 0x1F) * pulseBrightness;
                uint8_t g6 = ((jetColor >> 5) & 0x3F) * pulseBrightness;
                uint8_t b5 = (jetColor & 0x1F) * pulseBrightness;
                uint16_t finalColor = (r5 << 11) | (g6 << 5) | b5;
                
                tft.drawLine((int)jetX, (int)jetY, (int)endX, (int)endY, finalColor);
            }
        }
    }
    ...
    for (int i = 0; i < MAX_TRAPEZIUM_STARS + MAX_BACKGROUND_STARS; i++) {
        if (!orionStars[i].active) continue;
