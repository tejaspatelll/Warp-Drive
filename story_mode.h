...
extern bool factPopupActive; // Track if fact popup is currently shown
...
bool StoryMode::processInput(int potValue, bool buttonPinState_LOW)
{
    ...
    // Use the same warp threshold and calculation as discovery mode
    // Scale from 0-4095 to 0-1.0 for 12-bit ADC (same as discovery mode)
    float rawWarpFactor = static_cast<float>(potValue) / 4095.0f;
    this->currentWarpFactor = easeInOutCubic(rawWarpFactor);
    // Use a more precise threshold for 12-bit ADC (about 2.5% of full scale) - same as discovery mode
    ...
    if (shouldWarp && !previousWarpState)
    {
        ...
        warpUIFrameDrawn = false; // Reset flag to draw UI frame on next update
        uiFrameDrawn = false;     // Reset this too so drawPermanentUIFrame() will execute
        uiRedrawFrameCounter = 0; // Reset frame counter
        Serial.println("StoryMode: Entered Warp.");

        // Clear ONLY the viewport area, not the entire screen!
        tft.fillRect(viewportX, viewportY, viewportSize, viewportSize, BG_COLOR);

        // Initialize warp buffer for smooth animation
        initWarpBuffer();

        // Ensure viewport area in buffer is initialized to BG_COLOR (not black)
        // This is critical for story mode where we only update the viewport
        if (bufferInitialized && warpBuffer != nullptr)
        {
            for (int y = viewportY; y < viewportY + viewportSize; y++)
            {
                for (int x = viewportX; x < viewportX + viewportSize; x++)
                {
        ...
        // Initialize stars within the viewport for warp animation
        float vpCenterX = viewportX + viewportSize / 2.0f;
        float vpCenterY = viewportY + viewportSize / 2.0f;
        float maxRadius = sqrt(powf(viewportSize / 2.0f, 2) + powf(viewportSize / 2.0f, 2));

        for (int i = 0; i < STAR_COUNT; i++)
        {
            // Position stars randomly within viewport
            float angle = random(0, 360) * PI / 180.0f;
            float distance = random(10, (int)maxRadius);
            stars[i].realX = vpCenterX + cos(angle) * distance;
            stars[i].realY = vpCenterY + sin(angle) * distance;
    ...
    {
        // We're exiting warp mode - keep UI visible!
        Serial.println("StoryMode: Exiting Warp.");
