...
void drawSetDialToZeroPrompt(int potValue) {
    ...
    if (firstDraw) {
        ...
        // Instruction box with retro styling
...
// --- New: Draw a modern title bar with double-buffering approach ---
...
// --- New: Draw a modern narration box with improved approach ---
inline void prepareStoryNarrationSprite(const char* text) {
    ...
    while (start < stext.length()) {
        ...
        while (end < stext.length() && stext[end] == ' ') end++;
        start = end;
    }
    ...
    if (now - lastStoryScrollTime > storyScrollSpeed) {
        ...
        storyScrollY++;
        if (storyScrollY > lines.size() * 14 + boxH) storyScrollY = 0;
    }

    // Draw text with glow effect
    int y0 = boxH - 14 - storyScrollY;
    storyNarrationSprite.setTextDatum(ML_DATUM);
    
    for (size_t i = 0; i < lines.size(); ++i) {
        int lineY = y0 + i * 14;
        if (lineY >= -14 && lineY <= boxH) {
            // Glow effect
            storyNarrationSprite.setTextColor(tft.color565(100, 200, 255));
            storyNarrationSprite.drawString(lines[i], 11, lineY + 1);
            // Main text
            storyNarrationSprite.setTextColor(tft.color565(255, 255, 0));
            storyNarrationSprite.drawString(lines[i], 10, lineY);
        }
    }
}
