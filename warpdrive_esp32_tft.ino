...
void hapticFeedback(float warpFactor)
{
  ...
  {
    if (random(1000) < (warpFactor - 0.5f) * 200)
    { // Higher warp = more instabilities
      inInstability = true;
  ...
  else
  {
    ...
    if (now - lastPatternTime > patternDuration)
    {
      ...
      else if (stressHarmonics && substate < 3)
      {
        ...
        if (secBtn && !secondButtonPressed && millis() - secondLastButtonTime > SECOND_DEBOUNCE_DELAY)
        {
          ...
          if (!factPopupActive)
          {
            ...
            else
            {
              ...
              {
              case CelestialObject::STAR:
                factText = "Stars form from collapsing gas clouds; our Sun will burn its hydrogen for ~10 billion years.";
                break;
              case CelestialObject::PLANET:
                factText = "Planets range from rocky terrestrials to gas giants; some exoplanets orbit so close they sizzle.";
                break;
              case CelestialObject::NEBULA:
                factText = "Nebulae are stellar nurseries: glowing clouds of gas where new stars emerge.";
                break;
              case CelestialObject::GALAXY:
                factText = "Galaxies can host trillions of stars, vast clouds of gas, and mysterious dark matter.";
                break;
              case CelestialObject::SOLAR_SYSTEM:
                factText = "Our Solar System has 8 planets and countless comets, all bound by the Sun's gravity.";
                break;
              case CelestialObject::ASTEROID_FIELD:
                factText = "Asteroid belts are leftovers of planet formation, sculpted by planetary resonances.";
                break;
              case CelestialObject::BLACK_HOLE:
                factText = "Black holes warp spacetime; stellar ones form when massive stars collapse.";
                break;
              case CelestialObject::PULSAR:
                factText = "Pulsars are neutron stars spinning hundreds of times per second, beaming radiation like lighthouses.";
                break;
              case CelestialObject::SUPERNOVA:
                factText = "Supernovae briefly outshine galaxies and forge heavy elements like gold and uranium.";
                break;
              case CelestialObject::COMET:
                factText = "Comets are icy bodies that develop glowing tails of gas and dust when heated by the Sun.";
                break;
              case CelestialObject::BINARY_STAR:
                factText = "Binary stars orbit each other; close pairs can exchange material in dramatic outbursts.";
                break;
              case CelestialObject::SPACE_STATION:
                factText = "Space stations like the ISS let humans live in microgravity to advance science and tech.";
                break;
              case CelestialObject::JEWEL_BOX:
                factText = "The Jewel Box Cluster displays a stunning variety of colored stars in a compact open cluster.";
                break;
              case CelestialObject::OMEGA_CENTAURI:
                factText = "Omega Centauri is the largest globular cluster in our galaxy with millions of ancient stars.";
                break;
              case CelestialObject::ORION_NEBULA:
                factText = "The Orion Nebula (M42) is one of the brightest nebulae visible to the naked eye.";
                break;
              case CelestialObject::PLEIADES:
                factText = "The Pleiades (Seven Sisters) is a beautiful open star cluster with hot blue stars.";
                break;
              case CelestialObject::RING_NEBULA:
                factText = "The Ring Nebula (M57) is a planetary nebula with a distinctive donut/ring shape.";
                break;
              case CelestialObject::DOUBLE_CLUSTER:
                factText = "The Double Cluster (NGC 869/884) is two open clusters close together in Perseus.";
                break;
              default:
                factText = "";
                break;
...
// Add destroys for other sprites if they exist (nebula, planet etc)
      ...
      {
      case CelestialObject::STAR:
        playDiscoveryObjectSound("STAR");
        break;
      case CelestialObject::PLANET:
        playDiscoveryObjectSound("PLANET");
        break;
      case CelestialObject::NEBULA:
        playDiscoveryObjectSound("NEBULA");
        break;
      case CelestialObject::GALAXY:
        playDiscoveryObjectSound("GALAXY");
        break;
      case CelestialObject::BLACK_HOLE:
        playDiscoveryObjectSound("BLACK_HOLE");
        break;
      case CelestialObject::PULSAR:
        playDiscoveryObjectSound("PULSAR");
        break;
      case CelestialObject::SUPERNOVA:
        playDiscoveryObjectSound("SUPERNOVA");
        break;
      case CelestialObject::COMET:
        playDiscoveryObjectSound("COMET");
        break;
      case CelestialObject::BINARY_STAR:
        playDiscoveryObjectSound("BINARY_STAR");
        break;
      case CelestialObject::SPACE_STATION:
        playDiscoveryObjectSound("SPACE_STATION");
        break;
      case CelestialObject::JEWEL_BOX:
        playDiscoveryObjectSound("JEWEL_BOX");
        break;
      case CelestialObject::OMEGA_CENTAURI:
        playDiscoveryObjectSound("OMEGA_CENTAURI");
        break;
      case CelestialObject::ORION_NEBULA:
        playDiscoveryObjectSound("ORION_NEBULA");
        break;
      case CelestialObject::PLEIADES:
        playDiscoveryObjectSound("PLEIADES");
        break;
      case CelestialObject::RING_NEBULA:
        playDiscoveryObjectSound("RING_NEBULA");
        break;
      case CelestialObject::DOUBLE_CLUSTER:
        playDiscoveryObjectSound("DOUBLE_CLUSTER");
        break;
      default:
        break;
      }
...
void processMenuInput()
{
  ...
  if (secBtn && !secondButtonPressed && millis() - lastSecondButtonTime > 300)
  {
    ...
    {
      // Show popup
      settingsSelection = 0; // Reset selection to sound
      toggleSoundSettingsPopup();
    }
  }
  else if (!secBtn)
  ...
  if (digitalRead(BUTTON_PIN) == LOW && !buttonPressed)
  {
    ...
    { // Debounce
      ...
      {
      case State::DISCOVERY:
        playStateTransitionSound("DISCOVERY");
        break;
      case State::QUIZ:
        playStateTransitionSound("QUIZ");
        break;
      case State::STORY:
        playStateTransitionSound("STORY");
        break;
      default:
        break;
      }
...
void updateQuizMode()
{
  ...
  case QUIZ_QUESTION:
    ...
    if (quizState.selectedIndex == -1)
    { // Reset when no option is selected (e.g., after answering)
      lastQuizSelection = -1;
    }
...
void drawFactPopup()
{
  ...
  while (start < len && lineCount < 10)
  {
    ...
    while (end < len)
    {
      ...
      if (tft.textWidth(sub) > maxTextW)
        break;
      if (factText.charAt(end) == ' ')
        lastSpace = end;
      end++;
    }
    if (end == start)
    ...
    start = end;
    while (start < len && factText.charAt(start) == ' ')
      start++;
  }
...
void hideFactPopup()
{
  ...
  while (start < len && lineCount < 10)
  {
    ...
    lineCount++;
    start = end;
    while (start < len && tmp.charAt(start) == ' ')
      start++;
  }
  int boxH = lineCount * lineHeight + padding * 2;
  int boxX = (SCREEN_WIDTH - boxW) / 2;
...
void drawSoundSettingsPopup()
{
  ...
  int contentHeight = (toggleLabelHeight + toggleHeight) * 3 + toggleSpacing * 2 + powerOffSpacing + powerOffHeight; // Two toggles + spacing + labels
  ...
  // Then the main popup
  tft.fillRoundRect(popupX, popupY, popupWidth, popupHeight, 8, COLOR_TITLE_BG);
  tft.drawRoundRect(popupX, popupY, popupWidth, popupHeight, 8, COLOR_BORDER);
...
void drawSettingsToggle(int popupX, int toggleY, int popupWidth, int toggleWidth, int toggleHeight,
                        const char *label, bool enabled, bool selected)
{
  int toggleX = popupX + (popupWidth - toggleWidth) / 2;
  ...
  if (selected)
  {
    tft.drawRoundRect(toggleX - 2, toggleY - 2, toggleWidth + 4, toggleHeight + 4, 15, COLOR_HIGHLIGHT);
  }
  tft.fillRoundRect(toggleX, toggleY, toggleWidth, toggleHeight, 15, COLOR_MENU_GRID);
  tft.drawRoundRect(toggleX, toggleY, toggleWidth, toggleHeight, 15, COLOR_BORDER);

  // Toggle position based on state
  int knobSize = toggleHeight - 6;
  int knobX = toggleX + 3 + (enabled ? toggleWidth - knobSize - 6 : 0);

  // Draw toggle knob with 3D effect
  tft.fillRoundRect(knobX, toggleY + 3, knobSize, knobSize, 15,
...
// Add this function to toggle the sound settings popup
void toggleSoundSettingsPopup()
{
  soundSettingsPopupActive = !soundSettingsPopupActive;
