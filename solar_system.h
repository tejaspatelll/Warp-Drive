...
void drawSolarSystem()
{
    ...
    if (!solarSystemInitialized)
    {
        ...
        if (sunHandle.id == 0)
        {                                          // Check if sun sprite needs creation
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            {                                                   // Check if this planet sprite needs creation
    ...
    // Draw sun directly to screen (sprites disabled for better animation performance)
    float pulseFactor = (sin(t * 2.0f) + 1.0f) / 2.0f;

    // Erase previous sun if position or size changed
    if (prevSunRadius > 0 && (prevSunX != centerX || prevSunY != centerY || prevSunRadius != sunRadius))
    {
        tft.fillCircle(prevSunX, prevSunY, prevSunRadius + 3, BG_COLOR);
    }
    ...
    for (int r_sun_corona = sunRadius + 2; r_sun_corona > sunRadius; r_sun_corona--)
    {
        ...
        tft.drawCircle(centerX, centerY, r_sun_corona, coronaColor);
    }

    // Draw sun body
    uint16_t sunColor = tft.color565(255, 255, 0);
    if (flareActive)
    {
        sunColor = tft.color565(255, 255, 100);
    }
    tft.fillCircle(centerX, centerY, sunRadius, sunColor);

    prevSunX = centerX;
    prevSunY = centerY;
    prevSunRadius = sunRadius; // Store radius for next frame's erase

    // Orbit dots removed for cleaner appearance
    for (int i = 0; i < 4; i++)
    {
        prevOrbitRadii[i] = baseOrbitRadii[i] * objectScale; // Store for eraseSolarSystem()
    }

    // Draw planets directly to screen (sprites disabled for better animation performance)
    for (int i = 0; i < 4; i++)
    {
        ...
        // Erase previous planet if it moved
        if (prevPlanetRadius[i] > 0 && (prevPlanetX[i] != planetX || prevPlanetY[i] != planetY || prevPlanetRadius[i] != planetDrawRadius))
        {
            tft.fillCircle(prevPlanetX[i], prevPlanetY[i], prevPlanetRadius[i] + 1, BG_COLOR);
        }
        ...
        prevPlanetRadius[i] = planetDrawRadius;
