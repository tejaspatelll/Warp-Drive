 ...
 * EXIT ANIMATION FIX: Accounts for maximum possible scale during exit animation
 */
                               ...
                               int *pOrbitRadius1 = nullptr, int *pOrbitRadius2 = nullptr)
{
  ...
  // Calculate actual star radii based on sprite scale
  int radius1 = std::max(1, static_cast<int>(scale_i(7) * spriteScale * 0.8f));
  int radius2 = std::max(1, static_cast<int>(scale_i(4) * spriteScale * 0.8f));

  // Calculate orbit radii - this determines how far the stars are from center
  float orbitScaleVal = scale_f(15.0f) * spriteScale * 0.9f;
  ...
  if (pRadius2)
    *pRadius2 = radius2;
  if (pOrbitRadius1)
    *pOrbitRadius1 = orbitRadius1;
  if (pOrbitRadius2)
    *pOrbitRadius2 = orbitRadius2;
  ...
  float memoryRatio = (freePsram > 10000) ? std::min((float)freePsram / (float)requiredMem, 10.0f) : std::min((float)freeHeap / (float)requiredMem, 5.0f);
...
void drawBinaryStar()
{
  ...
  { // Create sprite if handle is invalid
// If memory is critically low, use direct drawing
  ...
  { // Handle is valid, try to get sprite reference
    spritePtr = SpriteManager::getSpriteRef(binaryStarHandle);
    if (!spritePtr)
    {
    ...
    if (abs(calculatedSpriteSize - spritePtr->width()) > 10)
    { // If size differs by > 10 pixels
...
void eraseBinaryStar()
{
  ...
  prevBinaryStarX = 0;
  prevBinaryStarY = 0;
  prevBinaryStarSize = 0; // Reset position info
}
