  ...
  for (int arm = 0; arm < MAX_GALAXY_ARMS; arm++)
  {
    for (int i = 0; i < prevGalaxyPointCount[arm]; i++)
    {
      ...
      if (x >= 0 && x < SCREEN_WIDTH && y >= 0 && y < SCREEN_HEIGHT)
      {
        ...
        if (x > 0)
          tft.drawPixel(x - 1, y, BG_COLOR);
        if (x < SCREEN_WIDTH - 1)
          tft.drawPixel(x + 1, y, BG_COLOR);
        if (y > 0)
          tft.drawPixel(x, y - 1, BG_COLOR);
  ...
  for (int arm = 0; arm < numArms; arm++)
  {
    ...
    for (int i = 0; i < pointsPerArm; i++)
    {
          ...
          pointsDrawn < MAX_GALAXY_POINTS)
      {
        ...
        if (random(15) == 0)
        { // Increased chance for colored stars
          if (distance > 0.7f)
          {
...
void drawAsteroidField()
{
  if (!asteroidFieldInitialized)
  {
    ...
    for (int i = 0; i < MAX_ASTEROIDS; i++)
    {
      ...
      // Scale speeds
      float speed = random(1, 4) * scaleFactor * objectScale + random(0, 100) / 100.0f;
      float angle = random(0, 360) * PI / 180.0f;
      asteroids[i].vx = cos(angle) * speed;
      asteroids[i].vy = sin(angle) * speed;

      // Scale sizes
      asteroids[i].radius = scale_i(random(1, 4)) * objectScale;
