...
void initializeRing() {
    ...
    // Also account for Orion Nebula's PSRAM usage if it gets initialized first
    // (Though in this sketch, only one object is active at a time, so total PSRAM is key)
    if (requiredSize > availablePSRAM) {
        numRingParticles = availablePSRAM / particleSize;
        if (numRingParticles < 50) numRingParticles = 50; // Ensure a minimum
    ...
    for (int i = 0; i < numRingParticles; i++) {
        ...
        if (zoneRand < 0.25f) { // 25% in inner zone (hottest, greenest)
        ...
        } else if (zoneRand < 0.65f) { // 40% in middle zone (peak brightness)
        ...
        } else { // 35% in outer zone (cooler, redder)
...
void drawRing() {
    ...
    if (globalExpansion > 2 * PI) globalExpansion -= 2 * PI;
    
    // Draw ring particles first
    for (int i = 0; i < numRingParticles; i++) {
        if (!ringParticles[i].active) continue;
    ...
    for (int i = 0; i <= MAX_BACKGROUND_STARS; i++) {
        if (!ringStars[i].active) continue;
